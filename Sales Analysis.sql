...
SELECT 
	...
	SUM(total_sale)/COUNT(o.order_id) as AOV, --AOV(Avg. order value)--
...
WHERE 
    o.order_status = 'Pending Shipment' -- Filter orders still pending shipment --
    AND p.payment_date IS NOT NULL; -- Ensure the order has been paid --
-- There is no data that has pending shipment --
...
BEGIN
    -- Fetch product name and price based on product_id --
    SELECT 
        price, product_name
    INTO
        v_price, v_product
    ...
    -- If stock is available, proceed with sales and update --
    IF v_count > 0 THEN
    ...
    ELSE
        -- Notify product is not available --
